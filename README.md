...
```ruby
logger.attribute(user_id: 123, request_id: "abc-def") do
...
class MyLogTaggingMiddleware
  ...
  def call(worker, job, queue)
    # Add attribute_1 to all logs for this job.
    Sidekiq.logger.attribute(attribute_1: job["value_1"]) if Sidekiq.logger.is_a?(Lumberjack::Logger)
